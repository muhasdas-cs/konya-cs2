...
def get_match_result(match_name):
    ...
    try:
        ...
        if len(team1_data) > 0:
            team1_score = int(team1_data[0].get("roundsWon", 0))

        if len(team2_data) > 0:
            team2_score = int(team2_data[0].get("roundsWon", 0))
...
for name in players_elo:
    ...
    adr = s["damage"] / s["matches"] if s["matches"] > 0 else 0
...
print("🔥 SYSTEM FULL FIXED - NO FAKE MATCHES")
